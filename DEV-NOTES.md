...
### Why audio is the only lever (researched at source level, 2026-09-26)

Both engines throttle a hidden page and both exempt a page that is playing audio — but they exempt by **different tests**, which is why the implementation is a Web Audio oscillator rather than an `<audio>` element:

- **Chromium** (`developer.chrome.com/blog/timer-throttling-in-chrome-88`, still accurate in 2026): *minimal* throttling — i.e. none — applies when the page is visible **or “has made noises in the past 30 seconds … but a silent audio track doesn't count”**. Otherwise timers run at 1 Hz, and after **5 hidden minutes** (the page “has been silent for at least 30 seconds”, chain count ≥ 5) they fall to **once a minute**. A page that is playing sound sits in the minimal group, so it is not throttled at all. WebRTC (an open `RTCDataChannel`) is the only other exemption, and it skips only the once-a-minute stage.
  - **What “makes noises” means for Web Audio** — `third_party/blink/renderer/modules/webaudio/audio_context.cc`: `bool IsAudible(const AudioBus* rendered_data) { … return energy > 0; }`, where energy is the summed squared amplitude of the render quantum. **Any non-zero sample at all is audible**; only exact silence is not. That is why `KEEP_AWAKE_GAIN` must never be `0`.
- **Firefox** (`dom/base/TimeoutManager.cpp`): `IsBackground()` = `!IsActive() && IsBackgroundInternal()`, and `IsActive()` returns true when `mGlobalObject.IsPlayingAudio()`. `MinSchedulingDelay()` returns 0 for an active manager, and budget throttling is disabled as well (`if (… || IsActive()) return false;`) — so a page that plays audio is **neither clamped nor budget-throttled** while hidden.
  - `nsGlobalWindowInner::IsPlayingAudio()` (`dom/base/nsGlobalWindowInner.cpp`) is true if **any `AudioContext` in the window `IsRunning()`** — *regardless of level* — or if `AudioChannelService::IsWindowActive()` says the window has audible agents (real media sound). In Firefox a running AudioContext alone is therefore enough; the gain could be 0 there. It is not, because Chromium needs non-zero energy.
  - The Firefox limits it defeats, for the record (`modules/libpref/init/StaticPrefList.yaml`): `dom.min_background_timeout_value = 1000`; `dom.timeout.enable_budget_timer_throttling = true` with a background budget of 50 ms regenerating at 1 ms per 100 ms and `dom.timeout.budget_throttling_max_delay = 15000`; and `layout.throttled_frame_rate = 1` (rAF drops to ~1 fps in a hidden tab, and is suspended outright in an occluded window). Firefox is much gentler than Chromium — a backgrounded run slows down rather than stalling.
  - Author's browser at the time of writing: **Firefox 156 / Windows** (recorded by the probe's `ua`).

### What was built

- **`comicGen.keepAwake`** (browser preference, **default OFF**): `KEEP_AWAKE_KEY` + `KEEP_AWAKE_GAIN = 0.0001` (−80 dBFS) beside the other header/pref keys. Deliberately per-browser and NOT part of the project file, exactly like `comicGen.bgGenerate` — a phone and a desktop want different answers here.
- **The whole surface:** `applyKeepAwake(on)`, `startKeepAwake()`, `stopKeepAwake()`, `keepAwakeRunning()`, `keepAwakeStateText()`, `updateKeepAwakeState()`, `keepAwakeResume()`, `keepAwakeArm()`, `keepAwakeUnarm()`, `keepAwakeGestureTry()`, `keepAwakeKick()` and `setKeepAwakeStatus()`. `onPrefKeepAwakeChange()` is the inline handler (mirroring `onPrefBgGenerateChange`), and all of it is exported on `window`.
- **The loop itself:** one `AudioContext` + a 220 Hz `OscillatorNode` → a `GainNode` at `KEEP_AWAKE_GAIN` → `ctx.destination`. Created on demand, closed in `stopKeepAwake()`. No audio element, no data URI, no asset.
- **Autoplay is handled honestly.** A browser will not start audio without a user gesture, so `applyKeepAwake(true)` tries immediately and, if the context is not `running`, **arms** a capture-phase `pointerdown`/`keydown`/`touchstart` listener that retries at the first real click and disarms on success. At boot the app never creates the context (it arms instead), which is also what keeps the console free of the “AudioContext was prevented from starting automatically” warning.
- **Self-healing:** a `visibilitychange` + `pageshow` listener (`keepAwakeKick`) resumes a context the browser suspended, and re-arms if it is missing.
- **UI:** a third `check-label` row in **Edit → Preferences → Generation**, under the background-generation row, with a small `#keepAwakeState` line (`.pref-state` CSS) reading `● off` / `● waiting for your first click on this page` / `● starting…` / `● running now — this page will not be slowed down`, plus a status-line message on every toggle.
- **Manual:** §Preferences gained a **Keep this tab awake while I work** paragraph next to **Generation**, the design-document version moved to 2026.09.26.6, and §12 gained the usual changelog bullet. Version stamp → 2026.09.26.6.
- **Harness:** `G14:76` (defaults to off, unchecked, `state=off`, label `● off`) and `G14:77` (both directions persist `"1"`/`"0"`, the state becomes running-or-starting, the label renders, the status line starts with 🔊, unticking returns `off`). Smoke is now **77 checks / 73 pass / 4 manual**. A new **reload-proof** probe, `devtests/keepawake-probe.page.js`, keeps its samples in `sessionStorage` and splits the hidden heartbeat gaps by whether keep-awake was on; it replaces the live-only `devtests/bgprobe.page.js` for the author's tab-switch measurement, because the editor reloads the preview on Save and the old probe's samples died with the page (which is exactly what happened to the first measurement).

### Verified live (2026-09-26)

- **The preference plumbing:** the row exists in `#preferencesPanel` with the checkbox unchecked on a browser with no stored key; `window.applyKeepAwake` / `startKeepAwake` / `stopKeepAwake` / `keepAwakeStateText` / `onPrefKeepAwakeChange` are all functions; toggling writes `comicGen.keepAwake` = `"1"` / `"0"` and mirrors the checkbox.
- **The oscillator is real:** after `applyKeepAwake(true)` the state reads `running`, i.e. `AudioContext.state === "running"` — the page is genuinely playing audio at 0.0001 gain, which is the condition both engines test.
- **Everything is parked and restored:** each test round was bracketed by a full map dump; the author's 20-key map (`1c13afe4`) comes back byte-identical after every runner, the preference key does not exist in their storage, and the probe writes only its own `sessionStorage` key (`yacbpg.awake.probe`) — a **`sessionStorage` write is not app state** (the smoke suite already uses `__smoke_at` for the same reason), but it is still cleaned up by `__awakeStop()`/`__awakeReset()`.
- **Suites:** core 14/0, differential 21/0 (4 modules), generation 18/0, fixtures 16/0, smoke 73/0/4, 0 perchance errors, `byteIdentical: true` after each.
- **Visual:** the new Preferences row was captured and inspected (dark theme, `#preferencesPanel` expanded) — the label wraps, the state line sits on its own line under it, and nothing overflows. The 14-view visual baseline is unchanged (it does not capture the Preferences panel).

### Gotchas for the next session

- **Never set `KEEP_AWAKE_GAIN` to `0`.** Chromium's Web Audio audibility test is literally `energy > 0`, so exact silence defeats the mechanism entirely. The constant is not a volume setting; it is the signal that says “this page is making noise”.
- **Default OFF is a deliberate choice.** Chrome shows its 🔊 playing-audio icon for any page that is audible, and this setting makes the page audible by definition; making that happen on first load without being asked would be rude. (Firefox shows nothing — its icon is driven by `AudioChannelService` *audible* media, which a silent Web Audio tone is not.)
...
Released 2026-09-26 (stamp 2026.09.26.4). Author request, from the R-08 answers, verbatim: **“The fact that generation work stops when the browser tab loses focus. If there's no way around this then we'll live with it, but I really hope there's a way around it. This makes me sigh, so, so much. I really hope there's a way around it.”** (and 7b, where the run panel is their #1 wanted interface win, for the same reason). Logged in `PENDING.md` on receipt; the R-round answers were sent the same session.

### What was built
- **`comicGen.bgGenerate`** (browser pref, **default ON**): `const BG_GEN_KEY = 'comicGen.bgGenerate'` + `let bgGeneratePref = true` beside the other header/pref keys (~line 1724). `applyBgGenerate(on)` writes the key and mirrors the checkbox; `onPrefBgGenerateChange()` is the inline handler and is exported on `window`.
- **The gate is one line in the `visibilitychange` handler:** `if (!document.hidden) return; if (bgGeneratePref) return;` — everything after it (set `pausedByVisibility`, `runSignal.fire()`, stop every `inFlightGen` entry, delete their partial images, mark the boxes `.paused`, clear the panel `<img>`s, write the "Stopped generations — tab went to background" status) is the OLD behaviour and now only runs when the pref is off. There is no other place that reacts to `document.hidden`.
- **Restore + UI:** the boot block reads `localStorage.getItem(BG_GEN_KEY) !== '0'` (**absent = on**, so nothing needs migrating and a fresh browser gets the new behaviour) and ticks `#prefBgGenerate`; the markup adds a `pref-sub` **Generation** section between *Header & menus* and *Recent files* with one `check-label` row.
- **Deliberately NOT in the project file.** `buildExportData` carries `menuFullscreen` / `genAlwaysVisible` / `hdrAllViews`; `bgGenerate` was left out on purpose — it is a device/browser characteristic (a phone wants the opposite of a desktop), so it stays a per-browser preference like the Recent-files spinner.
- **Manual:** the ■ Stop bullet in §Generating now says generation keeps going in the background and points at the new setting, and §Using the interface gained a **Generation** paragraph next to **Preferences**. Version stamp → 2026.09.26.4 with its own changelog bullet.
- **Why the pause existed at all:** it was added 2026-08-10 during the mobile memory-hardening pass (see the TROUBLESHOOTING NOTE below) after a suspected memory-pressure tab kill. That incident turned out to be stale session state, and the DOM-side half of that hardening (the IntersectionObserver image eviction + `loading="lazy"`) is what actually does the work — holding ~24 JPEG data URLs ≈ 3 MB in JS is not a memory problem. So defaulting the pause OFF is a deliberate reversal of a workaround, not a regression.

### Verified live (2026-09-26)
- **Pref plumbing:** `#prefBgGenerate` exists inside `#preferencesPanel`, is **checked on a fresh browser with no stored key**, and its section reads `Generation` between `Header & menus` and `Recent files`. Toggling it writes `comicGen.bgGenerate` = `"0"` / `"1"` and `window.onPrefBgGenerateChange` is a function.
- **The gate, tested by faking the visibility state** (`Object.defineProperty(document, 'hidden', {get: () => true})` + a dispatched `visibilitychange`, both removed afterwards): with `root.generateImage` stubbed to a promise that never settles and counts `.stop()` calls, `generateSinglePanel(1, {pos:'bg-test panel 1', neg:''})` was started and then the page was "hidden" — **pref ON: 0 stops, the box stayed `panel-img-box generating`, no status message, the generation stayed in flight.** Same setup with the pref switched **off: 2 stops, the box became `panel-img-box paused`, status "Stopped generations — tab went to background…"** — i.e. the old path is intact when asked for.
- **No persistent writes:** the test drove generation through the `runOverride` path (which deletes `promptHistoryRuns[i]`) and never resolved, so nothing was written; all 21 stored keys were parked first (`devtests/park.js`, hash `6548bd20`, read-back verified) and restored byte-identical afterwards with no stray keys and no `__*` leftovers.
- **Housekeeping:** no console errors, no `perchanceErrors`, no `syntaxErrors`; the page still builds 24 cards and `#panelCount` is untouched.

### Gotchas for the next session
- **Do not "fix" the pause back in.** If a bug report mentions backgrounded tabs, check `bgGeneratePref` FIRST. The old behaviour is one checkbox away and is documented as such.
- **This does not defeat the OS.** A phone that suspends the page (or kills it) stops the run no matter what the page asks for; the resume path (`resumePanel` + `runEndPanel`, §7 of AI-NOTES) is what makes that harmless — the next ⚡ continues from the interrupted panel and never re-renders finished ones.
- **The `runOverride` argument of `generateSinglePanel(i, {pos, neg})` is the clean way to test generation without touching project state** — it skips `buildPanelPrompt` AND the empty-panel `skipped` path, and it deletes `promptHistoryRuns[i]`, so a stubbed `root.generateImage` exercises the real render/abort machinery with nothing persisted.
- **Faking visibility is the only way to test this in the editor:** `document.hidden` and `document.visibilityState` can be redefined on the instance and `delete`d afterwards. The handler reads `document.hidden` only.

## BATCH 2026.09.26.3 — the refactor planning documents + the R-01 … R-08 question form (docs round — no behaviour change)

Released 2026-09-26 (stamp 2026.09.26.3). Author request, verbatim: "I have something pretty ambitious in mind.  I'd like you to create a document that is a road map for eventually refactoring the code.  What I really would like is to have this document cover the functions and functionality of the app, independently of user interface.  Maybe a second one that suggests user interface ideas once the functionality is fully mapped out.  You are free to use whatever tools you have at your disposal for this.  I'd like it to be human readable, so that I can look through it and see what all we've got.  My requests have all been essentially, functions that I wanted right then, without any user interface design in mind at all, so I am sure the whole thing is pretty tangled and crufty.  You'll have questions, I know!  I'd like you to package those into HTML document(s) using the question-form.html file as a guide, to make it easier for me to answer, the way we did for T-01 through T-05.  Go ahead and begin this right now.  Make sure to push the documents to the Github when necessary, including whatever info you'll want to have handy as handoff notes and the like.  Thank you!" Logged in `PENDING.md` on receipt (standing rule).

### What was delivered
- **`FUNCTION-MAP.md`** (repo root) — the app as capabilities, deliberately with **no reference to the interface**; ends with a 21-item invariants list, a capability ledger and a dead/legacy-surface table. This is the document a refactor must keep satisfying.
- **`REFACTOR-ROADMAP.md`** — measured current structure + a 36-row cluster map of `index.html`'s line ranges + platform constraints + a target `src/` module tree + six phases (P0 harness → P1 pure logic → P2 state layer → P3 commands → P4 services → P5 UI extraction → P6 redesign), each with steps, exit criteria and the `try { await import('./src/core/x.js') } catch { in-file fallback }` strangler pattern, plus a testing strategy and a risk register.
- **`UI-IDEAS.md`** — 17 proposals (A–Q) as Problem → Proposal → Depends on → Risk, a "what not to change" list and a 10-step sequence.
- **`src/refactor-form.html`** — the R-01 … R-08 form (8 tickets / 31 questions), a copy of `src/question-form.html` with only `<title>`, `FORM_META` (`id: yacbpg-refactor-2026-09-26`) and `TICKETS` swapped; the driver, the ✕ close path and the "send to `tickets/`" path are the original. Every `select` has a "(recommended)" option so "all defaults" is answerable. A new **✨ Use the recommended answers** button (with its own handler beside `#clearBtn`) fills the recommended option into every `select` that is still blank and ticks the recommended boxes of 2c, then `persist()`s — so the author can press ✨ then Send, or change any of them first. (18 of 21 selects get a value; 4b / 5b / 8e have no recommended option by design.) One driver fix: `mergeIndex()`'s row regex is now `/^\|\s*\[([A-Z]+-\d+)\]\(…\)/` (was `T-\d+`, which would have silently dropped R- rows out of `tickets/INDEX.md` on a second send). Two driver fixes: (a) `mergeIndex()`'s row regex is now `/^\|\s*\[([A-Z]+-\d+)\]\(…\)/` (was `T-\d+`, which would have silently dropped R- rows out of `tickets/INDEX.md` on a second send); (b) **draft restore was broken** — `applyDraft(q)` was called inside `buildQuestion()`, i.e. before the caller did `formRoot.appendChild(sec)`, so every `getElementById('ctl-…')` returned null and a half-finished form came back blank on reload. The render loop now appends first and calls `applyDraft(q)` after. (`src/question-form.html` still has the old placement — left alone on purpose.)
- **`questions/REFACTOR-ROUND-1.md`** — the same questions as plain markdown, so they survive even if the form is never opened.
- **`index.html` — the only code change: the dev-form opener.** `openDevForm(file, title)` (beside `window.openUserManual = openUserManual;`, ~line 8900) builds a one-off `#devFormOverlay` (fixed `inset:0`, title bar `#devFormOverlayTitle` + ✕ Close, `#devFormOverlayFrame`) and loads the file into the iframe as an **object URL blob** (same-origin → the form can read/write the app's `localStorage`, i.e. pick up `comicGen.githubToken`/`Owner`/`Repo`). A failed fetch shows "Could not load … press Save in the editor once". Exports: `window.openDevForm`, `window.__openDevForm`, `window.__openRefactorForm()`, `window.__openQuestionForm()`. **No menu entry anywhere** — it is a developer tool for the author and future sessions.
- **Version bookkeeping:** `CHANGELOG.md` top entry `2026.09.26.3` (docs-round bullets), `#embeddedVersion` in `index.html` kept in sync, `src/manual.html` version stamp bumped to 2026.09.26.3 with one docs-only bullet (help content otherwise unchanged). Handoff notes: `AI-NOTES.md` **§18** (documents, the form, the opener, and the invariants this round must not break).

### Verified live (2026-09-26)
- `await window.__openRefactorForm()` in the served page created `#devFormOverlay` + `#devFormOverlayFrame`, titled **"Refactor & UI Planning — answer form"**, with the iframe on a `blob:https://<publicId>.perchance.org/…` URL.
- Inside the iframe: `document.title` = "Comic Book Page Generator — Refactor & UI Planning", **8 `<h2>` sections (R-01 … R-08)**, **21 selects / 31 checkboxes / 38 textareas**, `#pageTitle` / `#pageIntro` / `#pageVersion` filled from `FORM_META`. The iframe's `localStorage` showed the **20 app keys**, i.e. it is genuinely same-origin and will pick up the saved GitHub settings.
- Driver exercised without sending: a select was set + a note typed, the **Preview** `<details>` was toggled and `#previewEl` rendered 6,936 chars starting `=== COMIC BOOK PAGE GENERATOR — REFACTOR & UI PLANNING ===` with an `[partially-answered]` `R-01` section, `#statusEl` read "1 of 31 answered", and a draft key `questionForm.draft.yacbpg-refactor-2026-09-26` appeared. The test draft was then deleted and the overlay closed. No console errors, no `perchanceErrors`, no `syntaxErrors`.
- The paper trail was checked on GitHub after each PUT with `?ref=<new sha>` (a plain GET right after a PUT can serve a stale cached copy).

### Gotchas for the next session
- **The refactor is NOT authorised yet.** The three documents are proposals; `PENDING.md` says the R-tickets are the input and the work waits for the author's answers. Do not start P1 because the roadmap exists.
- **The form writes to the repo when the author presses Send** — one markdown file per ticket plus `tickets/INDEX.md`, via `api.github.com` and the `comicGen.githubToken` the backup feature already stores. When testing the form, exercise the preview/draft path only; **never press Send** outside an intentional ticket write.
- **`openDevForm` must load the file as a blob URL, not `srcdoc`.** `srcdoc` iframes are opaque-origin in modern browsers, so the form could not read the app's `localStorage` and the author would have to paste the token in by hand — the fields exist as a fallback, but the blob path is the intended one.
- Unsaved `src/` files need the service worker; inside an in-app browser without one the fetch fails and the fallback message appears. Press Save once.
- The measured counts in the three documents (10,194 lines, 543 callables, 199 window exports, 251 ids, 224 inline handlers, …) were taken on 2026-09-26 and **drift with every release** — re-measure before quoting them in a plan.

## BATCH 2026.09.26.2 — batch ⟳ refresh across the selection + the whole-selection buttons in Edit → Panel Selection

Released 2026-09-26 (stamp 2026.09.26.2). Author request, verbatim: "If multiple panels are selected, and Refresh From Library is clicked, I would like all of the basic descriptions in the selected panels to refresh from the library. Also, I would like the Edit -> Panel Selection menu to have its own copies of all of the buttons that can affect multiple panels. Please perform the usual pre-implementation tasks first. Thank you!" Logged in `PENDING.md` on receipt (standing rule), recon done, six questions asked and answered in one line ("Defaults are all correct!") before any code was written.

### What was built
- **Batch ⟳ refresh.** `refreshPanelLineBase(kind, i, s)` gained a first line: `if (panelBatchMode(i) && panelSelection.size > 1) { batchRefreshPanelLineBases(null); return; }` — so the ⟳ chip inside any *selected* panel refreshes the whole selection, while 0–1 selected keeps the old single-line behaviour byte-identical. New `panelLibDescRefreshOne(kind, i, s)` returns `'done'` / `'skipped'` / `'none'` (`'skipped'` ONLY when the line has a selection but the library text is empty; `'none'` for an empty slot, which is not counted at all), and `batchRefreshPanelLineBases(selOverride)` loops the three character slots + the location line of every selected panel, calls `schedulePanelSave()` once, and writes the status line ("⟳ Refreshed 5 basic descriptions from your library across 4 panels. 1 line had no library description to refresh, so it was left alone.").
- **Only the library-backed text moves.** The refresh writes `ids.base` and clears `data-needSeed` per line, then calls the existing `refreshPanelLineDesc` for the `edited` badge / `differs` bar. The `.pl-line-field` "This panel — extra description" boxes and the Action prompt are never touched — the single-panel refresh never touched them either, and the author confirmed the scope ("all of the basic descriptions").
- **Chip labels teach the batch mode.** `updateSelectionUI()` now also walks `document.querySelectorAll('.pl-base-refresh')`, parses the panel index out of the id (`/-baseref-(\d+)/`) and sets "⟳ Refresh N panels" when its panel is in a 2+ selection, else the original "⟳ Refresh from library" — the same count-in-the-label idiom the ⚙ Panel chips use.
- **Edit → Panel Selection copies.** A new `.panel-header-btns` row (plus a `#selMenuHint` line) after the existing Select-all / Copy / Cut / Clear row carries eleven buttons: `selMenuGenBtn`, `selMenuGenToBtn`, `selMenuGenFromBtn`, `selMenuGenRangeBtn`, `selMenuDupBtn`, `selMenuAddBtn`, `selMenuMoveBtn`, `selMenuRefreshBtn`, `selMenuClearImgsBtn`, `selMenuClearBtn`, `selMenuDelBtn`. They have no owning panel, so `selMenuSelection()` returns the sorted live selection (or null) and `selMenuAction(kind)` dispatches to the SAME functions the in-panel chips call, with `anchor = sel[sel.length - 1]` — the LAST selected panel, which is what the batch paths (`batchAddPanels`'s `lastSel`, `panelGenerateToHereAction`'s `end`, the range prefill's `to`) already key off. `move` opens the anchor's reorder picker (`toggleReorderPicker(anchor)`) and scrolls it into view; `refresh` calls `batchRefreshPanelLineBases(sel)`; the three destructive kinds pass `{ noSingle: true }`.
- **`noSingle`.** `panelClearAction(i, opts)` and `panelClearImagesAction(i, opts)` now build their `buttons` array into a local `btns` and only push the `🎯 Only This Panel` escape when `!(opts && opts.noSingle)`; `panelDeleteAction(i, opts)` forwards `opts` to `batchDeletePanels(opts)`, which skips its own escape the same way. Every in-panel call site passes nothing, so the escape is unchanged there.
- **Greyed until something is selected.** `updateSelectionUI()` writes every copy's text (with the `… N Panels` counts for the multi forms) and `b.disabled = (n === 0)`, and hides `#selMenuHint` once anything is selected. The markup ships all eleven `disabled`, so they are inert from the first paint; a programmatic click with an empty selection writes "⚠ Select at least one panel first — these buttons act on the panel selection."
- **Copy fixes.** The Panel Selection help paragraph (and the shipped manual) listed ⇅ Move as if it were a ⚙ Panel chip and omitted ⚡ Generate All To Here and ⚡ Generate (x) to (y)… — all three are corrected, and the new button row and the batch ⟳ are documented in both places.

### Verified live (2026-09-26)
- **Batch refresh, 4 panels selected**, 3 with a library character and 2 with a library location: one click on panel 1's ⟳ refreshed exactly those 5 Basic Descriptions, wrote "⟳ Refreshed 5 basic descriptions from your library across 4 panels. 1 line had no library description to refresh, so it was left alone." (the skipped line was a second character slot whose library entry has an empty `desc`), left the unused slots and the unselected panel's stale text alone, cleared the `edited` badge on the refreshed lines, and the debounced save persisted the new text (`pages[1][1].chars[0].base` / `locBase` matched the DOM).
- **Labels:** "⟳ Refresh 4 panels" on the selected panels, "⟳ Refresh from library" on an unselected one; back to the original label after the selection is cleared.
- **Single-panel paths unchanged:** with 0 selected, ⟳ on panel 1 refreshed only its own line; with exactly 1 selected (panel 1) the same — panel 2 was untouched.
- **Menu copies:** labels and `disabled` verified at 0 and 4 selected; **⟳ Refresh descriptions** produced the same batch refresh; **⚡ Generate (x) to (y)…** opened prefilled `From 1 / to 4` with "→ panels 1–4 (4 panels)" and Cancel left the selection at 4; **🗑 Clear Images** / **🗑 Clear** / **🗑 Delete** dialogs carried no "🎯 Only This Panel" and applied (statuses "Cleared the images of 4 panels (panel 1, 2, 3, 4).", "Cleared 2 panels (panel 1, 3).", "Deleted 2 panels from Page 1."), while the in-panel `panelClearImagesAction(3)` and `panelDeleteAction(2)` with a 2+ selection still showed the escape; **⧉ Duplicate** took 4 → 6 (each copy after its original), **＋ Add Panel** took 6 → 8 (both after the last selected), **⇅ Move** opened the anchor's picker in group mode (8 positions, no other-page options on a single-page project) and moved the block to position 1; the ⚠ status line appears on a programmatic empty-selection click.
- **Generate paths, with `root.generateImage` stubbed** to a 1×1 GIF: selection [2,3] → only panel 3 produced an image ("generated 1, skipped 1" — panel 2 is blank), To Here from [1,2] → only panel 1, and a single selected panel generated just that panel. The set of panels with images was read from the `rep` class on `imgbox-panel-p-1` (see gotchas).
- **Layout:** `vision` on the section at 390 px found all 15 buttons wrapped, greyed and unclipped, with the title, paragraphs and hint legible; at 1440 px all eleven were enabled with the count labels and the hint line hidden.
- **Storage:** all 20 stored keys were parked in `__test_backup_v10` **and** in `scratch/test-v10-backup-raw.json`, read back, then restored byte-for-byte (FNV-1a of the whole map `1c13afe4` before and after a full page reload, 0 mismatches, only the parked key removed, no stray keys, no perchance errors).

### Gotchas for the next session
- **`panelSelection` is not persisted** — but a batch refresh deliberately DOES persist (it edits real panel state via `schedulePanelSave()`), so any test that exercises it is a synthetic write and needs the park/restore protocol.
- **Selecting all panels and pressing 🗑 Delete on a single-page project resets the ENTIRE project** (`batchDeletePanels` → `confirm()` → `resetEverything(true)`). It does NOT delete the `comicGen.*` keys other than the project state (the token / owner / repo / library survived), but it wipes the throwaway project's content — test `Delete` on a sub-selection so at least one panel stays unselected.
- **`panelImages` is still module-scoped, not on `window`.** `updateRepHighlight(i)` toggles a `rep` class on `imgbox-panel-i-1` whenever slot 1 holds an image — that is a reliable DOM-side signal that does not depend on `imgObserver` scrolling (the `img-panel-i-k` `src` attributes only appear within ~400px of the viewport).
- **Stub `root.generateImage` to test anything that generates.** It is a plain property read on the pjs tree in `renderPanelSlot` (`pending = root.generateImage(opts)`), so `root.generateImage = async () => ({ dataUrl: FAKE })` works and the run completes instantly; a missing `.stop` on the returned promise is fine (every call site guards with `typeof pending.stop === 'function'`).
- **Cancel on a `showChoiceDialog` leaves the selection alone** (verified for the range, clear and delete dialogs) — a reading of "0 panels selected" after Cancel is usually the test's own helper having called `selectAllPanels()` + `clearPanelSelection()` first.
- **A closed dialog keeps its buttons in the DOM.** `#choiceBtns` is only hidden, never emptied, so reading `#choiceBtns button` after an action that did NOT open a dialog (e.g. `panelDeleteAction` with 1 selected, which deletes immediately) returns the PREVIOUS dialog's labels. Check the overlay's `hidden` state too.
