...
- **Follow-up (author, 2026-09-24, verbatim):** "No worries about the loaded project. \"Cow in field\" is
  ...
  work. **Restated at the author's request on 2026-09-25 (ticket T-02): anything loaded under that name can be
  clobbered at any time** — by a test run, a future AI session, or a stray import — so never park anything in it
  that you would mind losing, and never read it as a source of truth for the author's real work. Keep it in mind
  whenever a test needs a live project state — but the protocol below still applies to any other project, and the
  safest habit is still to back the bytes up before touching them.
