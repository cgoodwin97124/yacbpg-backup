...
## 2026.09.24.3 — 2026-09-24 — ↩ Restore Previous Project (one-step undo for New Project / Reset) + the library can never be deleted without its own confirmation
- **New Project and Reset now snapshot the project they are about to clear**, so an accidental wipe is recoverable. The snapshot keeps everything: every page and panel, titles, seeds, keywords, presets, 📝 Prompt overrides, the 🕘 Generated Prompt histories, image protection — **and your saved library**. Put it back with the new **↩ Restore Previous Project** button in **📄 File** (right after New Project) or in the **Reset to Defaults** panel; both name the snapshot (its project name, when it was taken, how many images it held) so you can see what you are about to bring back.
- **Generated images come back too** when the snapshot is still in the page's memory. The snapshot is also stored in your browser, so after a reload the project, the settings and the library can still be restored (the images, which only ever live in the page, cannot). Your own work is never overwritten silently: restoring always asks first.
...
- The reset confirmation is now the app's standard non-blocking dialog that lists exactly what will be cleared (and says so in the Reset to Defaults panel, whose “this cannot be undone” warning is gone now that a snapshot is kept).
