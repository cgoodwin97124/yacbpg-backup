...
## 2026.09.26.6 — 2026-09-26 — 🔊 A silent setting that stops your browser slowing the page down in a background tab

- **New setting: Edit → Preferences → Generation → “Keep this tab awake while I work”**, off by default. It plays a **sound you cannot hear** (a tone produced far below the audible level), and that is what stops the browser slowing the page down while you are in another tab, window or app. Both halves of the "switching away" problem are now covered: the setting above it stops the app *pausing* a run, and this one stops the **browser** *slowing the page down* — which affects the run in progress **and** the AI helper's own work.
- **Why audio, of all things.** A browser deliberately throttles a hidden page: timers drop to about once a second, animations stop, and Chrome cuts timers to about once a minute after five hidden minutes. But there is one documented exception in both Firefox and Chrome — **a page that is playing audio is left alone**. A quiet tone is therefore the only lever that actually defeats it, and it has to be a *real, non-silent* one: a silent track does not count.
- **It cannot be heard, and it cannot sneak up on you.** The tone is generated at **0.01% of full volume** — far below anything a speaker can reproduce in a normal room — and there is nothing to listen for. It starts at **your first click on the page** (browsers do not let a page make sound before you have interacted with it, so the setting shows “waiting for your first click” until then) and it stops the moment you untick the box. Because a quiet tone is still, technically, audio, **your browser's tab may show its 🔊 playing-audio icon** while the setting is on — that icon *is* the exemption, so it is working as intended.
- **Firefox users are covered.** Firefox has the same rule, and its own source is explicit about it: a window that is playing audio counts as “active”, which switches off both its one-second clamp and its budget throttling. So the same setting works whether you are in Firefox or Chrome.
- **Checked, not assumed.** 77 smoke checks (73 automated, 4 manual), 18 generation checks, 16 fixture checks, 14 pure-function checks and 21 differential checks pass, the 14-screenshot visual baseline is unchanged, and your project restores byte-identical after every suite.

## 2026.09.26.5 — 2026-09-26 — Refactor step 1: the pure logic moves into its own files (no behaviour change)

- **Nothing looks or behaves differently — this is the first step of the refactor plan you approved.** The .zip writer/reader, the small JSON scanner behind the project editor, the keyword/NSFW composition, and the seed arithmetic now live in four small standalone files (`src/core/zip.js`, `src/core/jsontext.js`, `src/core/keywords.js`, `src/core/seeds.js`) that the page loads at startup. The old copies stay inside the page as a fallback, so if a module cannot load (for example in a browser that cannot read unsaved `src/` files yet) the app quietly keeps using the built-in copy.
- **Why these four first.** They are the parts that must never be wrong: the seed maths that makes a page reproducible, the zip that carries your images, the scanner behind the hand-editable project JSON, and the keyword rules that add the safety terms while NSFW is off. Pulling them out is what makes them directly testable, and they are now checked by a *differential* suite that runs each module and the built-in copy side by side over thousands of generated inputs — 300 CRC buffers, 120 zips with 480 cross-reads, 600 JSON texts (229 of them deliberately corrupted, so the error messages and positions are compared too), 400 keyword/NSFW combinations, 400 seed inputs, 200 scrubbed documents — and finds zero differences. `library-core` and `prompt` are the two modules still to come.
- **The GitHub backup now carries the whole `src/` tree.** It used to upload only `src/manual.html`; it now pushes every file on its `GH_SRC_FILES` list (the manual, the two form files, and `src/core/*.js`), and the differential suite fails if an extracted module is missing from that list, so a new module cannot be silently left out of the backup.
- **Checked, not assumed.** 75 smoke checks, 18 generation checks, 16 fixture checks, 14 pure-function checks and 21 differential checks pass, and the 14-screenshot visual baseline is unchanged: the light-theme views that looked different on the first comparison turned out to be an artefact of capture order, and re-captured in the baseline's order they match pixel-for-pixel. Your project restores byte-identical after every suite.
