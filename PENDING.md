...
- **THE LIST (grouped by what unlocks it; the R-tickets have the full text and the answers):**
  ...
  - **Library (P2/P4):** each project keeps its own library, with an explicit way to pull entries from the browser-wide catalogue (4c — the trade-off is written up in `REFACTOR-NOTES.md`; the author asked to be talked through it before it is built).
  - **Images (P4/P6):** instead of a rolling per-slot history (measured cost ≈ 121 KB per kept image, so a "last 3 per slot" on a full page ≈ 35 MB — rejected, see `REFACTOR-NOTES.md`), a **⤓ Keep** action that pins an image *with the exact prompt text and seed that produced it*, so editing library descriptions can never lose a render they liked (5c).
  ...
  - **New domain idea (needs design):** per-panel **character dialogue** held as text in the project and *not* rendered into the image (8b).
