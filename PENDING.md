...
- **Measurement:** the passive probe that lost its samples to the editor's reload-on-Save has been replaced by the reload-proof `devtests/keepawake-probe.page.js` (samples kept in `sessionStorage`, hidden heartbeat gaps split by whether the preference was on). It is installed in the live preview and waits for the author's next switch-away; the numbers are the only outstanding item.
