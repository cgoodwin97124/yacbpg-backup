...
- **TICKET:** T-01 — tickets/T-01-generate-to-here.md ✅ answered 2026-09-25 (answers + notes live in the ticket; implementation still awaits the author's go-ahead)
...
- **CAVEAT FOUND:** when a run is driven by an explicit `panelList`, pausing stores `resumePanel = i` and the "continue" path rebuilds `seq` from `resumePanel` to the END OF THE PAGE — so after pausing a "To Here" run, ⚡ Generate All would continue *past* the clicked panel. Same pre-existing quirk as the batch-selection path; needs a decision (see the questions).
- **MARKUP:** the panel card's generate chips are built in `buildPanelGrid` (~3721): `🔄 Generate` (`#panel-gen-btn-i` → `panelGenerateAction(i)`) then `⚡ Generate All From Here` (`#panel-genfrom-btn-i` → `panelGenerateFromHereAction(i)`). The new chip (`#panel-gento-btn-i`) goes between them; the handler gets a `window.*` export next to `panelGenerateFromHereAction` (~8103). Proposed label `⚡ Generate All To Here` for symmetry.
- **QUESTIONS (2026-09-25) — ✅ ANSWERED 2026-09-25** (the questions and the author's answers, notes included, are recorded in the T-0x ticket file linked above):
  1. Label: "⚡ Generate All To Here" (to sit symmetrically with the existing "⚡ Generate All From Here"), placed between 🔄 Generate and ⚡ Generate All From Here?
  2. With several panels multi-selected, ⚡ Generate All From Here mirrors the selection and starts from the FIRST selected panel. Should ⚡ Generate All To Here mirror it too, finishing at the LAST selected panel (i.e. 1 … last-selected)? Or always be literal (1 … clicked)?
  3. After pausing a To Here run, ⚡ Generate All currently resumes to the end of the page (pre-existing quirk of the explicit-list path). Leave it, or make it remember the To Here end panel?
- **IMPLEMENTATION:** not started (awaiting the author's go-ahead).

### 2026-09-25 — DOC NOTE: "Cow in field" is a throwaway test project and may be clobbered at any time
- **TICKET:** T-02 — tickets/T-02-cow-in-field-throwaway-note.md ✅ answered 2026-09-25 (answers + notes live in the ticket; implementation still awaits the author's go-ahead)
- **Status:** 🕒 **QUEUED 2026-09-25** — logged on request; RECON in progress. **No code written** — the author explicitly said to hold implementation until they give the go-ahead.
...
- **ALSO:** the same one-liner belongs in the repo docs that get read before touching live state — `PENDING.md`'s test-protocol note and `ISSUES.md`'s post-mortem (which is where the incident that created the protocol is written up). Note there is deliberately NO `src/README.md` in this project: the author's standing rule forbids creating README/PENDING/AI-NOTES/CHANGELOG/ISSUES inside `src/` (it wedges platform saves), and the docs live in the GitHub repo.
- **QUESTIONS (2026-09-25) — ✅ ANSWERED 2026-09-25** (the questions and the author's answers, notes included, are recorded in the T-0x ticket file linked above):
  1. The note is documentation only (no code, nothing shipped): add it now, or at the same time as the go-ahead for the rest?
- **IMPLEMENTATION:** not started (awaiting the author's go-ahead).

### 2026-09-25 — FEATURE: how many recent files are kept must be user-configurable (Edit → Preferences)
- **TICKET:** T-05 — tickets/T-05-recent-count-preference.md ✅ answered 2026-09-25 (answers + notes live in the ticket; implementation still awaits the author's go-ahead)
- **Status:** 🕒 **QUEUED 2026-09-25** — logged on request; RECON in progress. **No code written** — the author explicitly said to hold implementation until they give the go-ahead.
...
- **RECON (2026-09-25):** the Preferences panel is `#preferencesPanel` in the Edit menu (~846) and already contains both shapes a knob can take: `check-label` checkboxes wired to `onPref*Change()` handlers that persist a `comicGen.*` '1'/'0' key (`prefMenuFullscreen`, `prefGenAlwaysVisible`, `prefHdrAllViews`, `hidePasswordPref`), and `pref-row` rows holding a `<select>` (theme mode) or a color input (accent). The count knob is a small `pref-row` — a `<select>` of 1-20 (or a number input) — persisted as a BROWSER-level preference (`comicGen.recentMax`, default ~5), not part of the project, because the Recent list itself is browser-level (file handles are per-browser). It is read when the list renders and when an entry is added (keep newest N); lowering it should trim the stored list immediately. Should the Recent list also be hidden entirely when the count is 0? (question)
- **QUESTIONS (2026-09-25) — ✅ ANSWERED 2026-09-25** (the questions and the author's answers, notes included, are recorded in the T-0x ticket file linked above):
  1. Knob shape: a <select> of 1-20 (default 5), or a free number box with a sane cap? Should 0 hide the Recent list entirely?
  2. Stored as a browser preference (comicGen.recentMax), not with the project — agreed? (It cannot be project data: the handles it counts are per-browser.)
- **IMPLEMENTATION:** not started (awaiting the author's go-ahead).

### 2026-09-25 — FEATURE: "Recent" list under 📄 File — reopen a saved project (JSON or ZIP) without the system file dialog
- **TICKET:** T-03 — tickets/T-03-recent-list-under-file.md ✅ answered 2026-09-25 (answers + notes live in the ticket; implementation still awaits the author's go-ahead)
- **Status:** 🕒 **QUEUED 2026-09-25** — logged on request; RECON in progress. **No code written** — the author explicitly said to hold implementation until they give the go-ahead.
...
- **RECON (2026-09-25):**
  - **What exists today:** `saveSettingsAs()` (~6497) uses `window.showSaveFilePicker()` and remembers exactly ONE target — `saveHandle` in memory plus `{handle, name}` in the IndexedDB store `comicGenSaveState` (`idbSet('handle'|'name')`, restored by `restoreSaveState()` at boot ~6588). `ensureWritePermission(handle)` already knows how to re-grant permission after a page reload (`queryPermission` → `requestPermission`), and a click on a Recent entry is exactly the user gesture `requestPermission` requires. A Recent list is the same machinery with an array of entries under a second key (e.g. `'recent'`), each `{handle, name, at}`.
  - **Opening one:** `handle.getFile()` returns a normal `File`, and the existing pipeline takes it as-is — `importSettingsFromFile()` (~6947) branches on the `.zip` name/type, calls `doImportFile(file, isZip)`, and shows the standard `#importConfirmOverlay` warning (Save / Continue / Cancel) whenever `hasActiveProject()`. ZIP reading is already handled by `unzipEntries()` + `parseZipImages()`. So a Recent entry reuses the whole flow, warning included, verbatim. The only new failure mode is a file that has since been moved/renamed/deleted: `getFile()` then throws (NotFoundError) and needs a friendly message plus removal from the list.
  - **Real limits to decide on:** (1) browsers deliberately do not expose a file's directory — a `FileSystemFileHandle` only knows its own filename — so "the path" can realistically be the filename plus when it was saved/opened (the *header* could show the folder only if we stored a directory handle instead, which we never get from a file picker); (2) only files the app itself opened or saved through a picker can be remembered — `⬆ Import Project` is a plain `<input type=file>` (no handle) and `💾 Save…` in the download-fallback path has no handle either; (3) the app only ever SAVES JSON (`showSaveFilePicker` accepts `.json`), while ZIPs are downloads — so for the list to hold ZIPs at all it needs an "add/open" picker (`showOpenFilePicker` accepting `.json` + `.zip`), which is also the natural home for a small `＋ Add…` button.
  - **Storage:** browser-level (IndexedDB), never part of the project — it must stay out of `panelState`, the 🧩 JSON editor and Export / Import, exactly like `comicGen.promptThumbs`. It also cannot survive a different browser/profile, and clearing site data empties it.
- **QUESTIONS (2026-09-25) — ✅ ANSWERED 2026-09-25** (the questions and the author's answers, notes included, are recorded in the T-0x ticket file linked above):
  1. Browsers deliberately hide a file’s directory, so the list can show only the FILENAME plus when it was saved/opened — not a real path. Is "filename — saved <time>" enough, or would you like a "&#65291; Add…" entry using the browser’s open-file picker (which can pick .json AND .zip) as the way files get into the list?
  2. Should the list hold ZIPs as well as JSON? It cannot otherwise: the app only ever SAVES JSON (ZIPs are downloads), so a ZIP can only reach the list through that picker.
  3. Placement: directly under the Backup Project chip row in 📄 File? Plus a "Clear recent list" action?
  4. Default number kept (5?) — see the Preferences entry above.
- **IMPLEMENTATION:** not started (awaiting the author's go-ahead).
...
- **TICKET:** T-04 — tickets/T-04-basic-description.md ✅ answered 2026-09-25 (answers + notes live in the ticket; implementation still awaits the author's go-ahead)
- **Status:** 🕒 **QUEUED 2026-09-25** — logged on request; RECON in progress. **No code written** — the author explicitly said to hold implementation until they give the go-ahead.
...
- **STATE / where it must travel:** per panel, `chars: [{sel, extra}]` and `loc` + `locExtra` inside `panelState` (~2919 collect, ~3444 restore) — already saved with the project, included in Export / Import and the 🧩 JSON editor. Supporting pieces that all assume the present two-field shape: `migratePanelExtraCopies(page)` (~3380, clears an `extra` that is byte-equal to the library description — a de-dupe that becomes meaningless once an edit means "override"); `readPanelItem()` / `panelItemIsSet()` (~3507, the ⇤ source search — an item counts as present when `sel !== 'none'`); `copyFromPrevPanel()` (~3553, copies `sel` + `extra`); `deletePanelChar/Loc` and `clearPanelImage` (~7045, blank both); and `newPanelLibraryEntry()` (~1333, prompts for a per-panel description when a library item is created from inside a panel).
- **OPEN DESIGN POINTS (questions in chat):** whether the separate `extra` field is merged away or kept alongside the new Basic Description; how existing projects' `extra` values migrate so their prompts do not silently change; whether an untouched Basic Description keeps following later edits to the library object or freezes at the text it was filled with; whether built-in characters/locations get the identical treatment; the exact "differs" comparison (trimmed, case-sensitive vs case-insensitive); and how the differing state is called out in the collapsible line (border colour, badge, chip).
- **QUESTIONS (2026-09-25) — ✅ ANSWERED 2026-09-25** (the questions and the author's answers, notes included, are recorded in the T-0x ticket file linked above):
  1. Merge or keep both fields? RECOMMENDED: merge — the read-only "Library description" becomes the editable "Basic Description", and the separate "This panel — extra description" textarea disappears (editing the Basic Description is what customising a panel now means). The alternative is to keep the extra field as an additional appended fragment, which leaves two overlapping ways to customise.
  2. Migration for existing projects: set each Basic Description to "library description + extra" (reproducing today’s prompt exactly, and flagging those as differing)? RECOMMENDED yes — the alternative silently drops existing extras from prompts.
  3. If you later edit a library object’s description, should an UNTOUCHED Basic Description follow the new text (RECOMMENDED — it is still "inherited"), or stay frozen at the text it was filled with?
  4. Built-in characters/locations (Hero, City, …) — identical editable/override/call-out treatment? RECOMMENDED yes.
  5. "Differs" comparison: trimmed, exact (case-sensitive)? RECOMMENDED yes.
  6. Call-out style for a differing Basic Description: a small "edited"/"custom" badge next to the label, plus a coloured left border on the field? (The line is inside a collapsible section, so the call-out should also be visible while collapsed — a dot on the ▸/▾ toggle row?)
- **IMPLEMENTATION:** not started (awaiting the author's go-ahead).
