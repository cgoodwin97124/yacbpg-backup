...
- **Status:** 🕒 **QUEUED 2026-09-26** — logged on receipt (2026-08-13 rule). RECON done and clarifying questions sent to the author the same day; **do NOT implement until the author answers** (2026-09-20 standing directive).
...
- **RECON (2026-09-26) — `⟳ Refresh from library`:**
  ...
  - Batch idiom already in the codebase: `panelBatchMode(i)` is `panelSelection.size > 0 && panelSelection.has(i)`; every selection-aware ⚙ Panel button does `if (panelBatchMode(i) && panelSelection.size > 1) { …for (const p of panelSelectionSorted())… }`, and `updatePanelSelectionUI()` (~4396) rewrites each button's label to carry the count (e.g. "🔄 Generate 3").
  - `dataset.needSeed` is the wrinkle: a base box marked `needSeed==='1'` is auto-filled from the library the first time it is needed (`seedPendingPanelBases`), and a single-panel refresh on a "none"/placeholder line blanks the box and re-marks it. A batch refresh therefore needs an explicit policy for lines with no library-backed selection (question 2 below).
- **RECON (2026-09-26) — Edit → Panel Selection menu:**
  - The section (index.html ~865, inside 🎨 Edit) holds only four buttons today: ☑ Select all panels, 📋 Copy, ✂ Cut, ✕ Clear selection.
  - Everything that can act on more than one panel, and where it lives now — the ⚙ Panel accordion: 🔄 Generate (`#panel-gen-btn-i` → `panelGenerateAction`), ⚡ Generate All To Here (`#panel-gento-btn-i`), ⚡ Generate All From Here (`#panel-genfrom-btn-i`), ⚡ Generate (x) to (y)… (`#panel-genrange-btn-i`), ⧉ Duplicate (`#panel-dup-btn-i` → `batchDuplicatePanels`), ＋ Add Panel (`#panel-add-btn-i` → `batchAddPanels`), 🗑 Clear Images (`#panel-clear-imgs-btn-i`), 🗑 Clear (`#panel-clear-btn-i`), 🗑 Delete (`#panel-del-btn-i` → `batchDeletePanels`); and in the panel's title bar, ⇅ Move (`#reorder-btn-i` → `toggleReorderPicker`, which moves the selection as a block) plus the conditional 📋 Paste chip. Each gates on `panelBatchMode(i)` and uses its owning panel's index `i` as the anchor — Duplicate inserts after the **last** selected panel, Add Panel adds one empty panel per selected panel after the last, To Here ends at the last selected, From Here starts at the first selected, the range picker prefills first…last, and Delete lets `batchDeletePanels` choose how the page refills. The top bar's ⚡ Generate All / ⏸ Pause / ■ Stop act on the whole page (not the selection) and stay reachable while the menu is hidden, so they look out of scope.
  - Stale help text spotted while reconning: the Panel Selection paragraph (index.html ~866) lists ⇅ Move as if it were one of the ⚙ Panel buttons (it is a title-bar button) and omits ⚡ Generate All To Here and ⚡ Generate (x) to (y)… from the selection-aware list — worth correcting in the same pass.
  - A Panel Selection menu button has no owning panel, so the position-aware actions (Duplicate / Add / Move / To Here / range prefill) need a defined anchor — see question 5.
- **DESIGN QUESTIONS SENT TO THE AUTHOR (2026-09-26, with proposed defaults in brackets) — awaiting answers:**
  1. Scope and trigger for the batch refresh: from the ⟳ button inside a panel's 📖 Panel Library, when 2+ panels are selected and this panel is one of them, refresh the **Basic Description** of every character slot **and** the location line of every selected panel — leaving the "This panel — extra description" boxes and the Action prompt alone — and keep exactly today's single-panel behaviour with 0–1 selected? [yes]
  ...
  3. Should the Panel Selection menu also get its own ⟳ refresh button (so it can be used without opening a panel's accordion)? [yes — the same action, which needs no owning panel]
  4. Which buttons to copy into the menu — proposed: 🔄 Generate, ⚡ Generate All To Here, ⚡ Generate All From Here, ⚡ Generate (x) to (y)…, ⧉ Duplicate, ＋ Add Panel, ⇅ Move, 🗑 Clear Images, 🗑 Clear, 🗑 Delete, plus ⟳ Refresh descriptions; anything to add or drop? [proposed list; the menu's existing ☑ Select all / 📋 Copy / ✂ Cut / ✕ Clear selection stay]
  5. Anchor for the position-aware copies (Duplicate / Add Panel / Move / To Here / range prefill), now that no panel owns the click: use the **last selected panel**, i.e. what the batch paths already do? [yes]
  6. With nothing selected: disable the new buttons (greyed, "select some panels first" tooltip) rather than falling back to a single-panel action; the destructive copies keep their existing confirmations but without the "🎯 Only This Panel" escape (there is no owning panel); and the labels carry the selection count like the in-panel buttons ("🔄 Generate 3", "🗑 Delete 3 Panels")? [yes to all three]
