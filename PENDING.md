...
- **Status:** 🟢 **START NOW 2026-09-26** — the author said "Go ahead and begin this right now." Docs-only (no app behaviour changes planned in this pass); any refactor that follows waits for the author's answers to the question form.
...
- **WHAT IS BEING DELIVERED (all in the repo, so they are human-readable on GitHub and survive this session):**
  1. **`FUNCTION-MAP.md`** — the app described as *capabilities*, with no reference to the interface: every subsystem (project/page/panel data model, generation pipeline, prompt building, seeds, the library, images and their per-slot flags, persistence, export/import, backups, multi-page reflow, selection/clipboard, prompt history + thumbnails, the JSON document, the manual/changelog, the ticket form, the GitHub backup), what each one *guarantees*, the state each one owns, and the invariants that must survive a refactor.
  2. **`REFACTOR-ROADMAP.md`** — the map of the *current* code structure (measured, not guessed: function counts, the biggest functions, the duplicated patterns, the global surface, the DOM-as-state-model), the target architecture, and a phased, reversible migration plan whose phases each end in a shippable, verifiable state.
  3. **`UI-IDEAS.md`** — interface proposals that only make sense once the functionality is mapped: the panel header/menu pile-up, edit-vs-generate separation, the accordion maze, selection-as-a-mode, a real inspector, keyboard-first flows, mobile layout, progressive disclosure, and a coherent destructive-action system.
  4. **`src/refactor-form.html`** — the questions as a fill-in form built on the same driver as `src/question-form.html` (same select / checkboxes / text question types, same ✕ close button and same "write the answers into the repo" send path), opened with `window.__openRefactorForm()` on the preview page. Its answers come back as ticket files, and the follow-up work is planned from them.
- **RECON (2026-09-26, measured):** `index.html` is one classic IIFE — ~10,400 lines / ~526 KB, no modules, no build step, all of it shipped to every visitor. It contains ~560 function declarations, ~330 `window.*` exports (the de-facto public API, because every inline `onclick=` needs one), ~150 `document.getElementById`-addressable element ids, and the living state is split across module-scope variables, the DOM itself (textarea/select values are read back on every collect), `comicGen.*` localStorage keys, and a per-page in-memory image store. Main.pjs holds only the pjs metadata + the `text-to-image-plugin` import; `src/manual.html` and `src/question-form.html` are the only other shipped files. Full findings, the target architecture and the migration phases live in the two documents themselves.
- **PLAN:** recon → write the three documents → build and test the question form → commit the docs + form + a copy of the questions to the repo → update `AI-NOTES.md` / `DEV-NOTES.md` with the handoff pointers → push the backup → open the form on the preview page for the author and WAIT for the answers before any refactoring starts.
