...
- **Status:** item 1 ✅ DONE (2026.09.23.10); item 2 ✅ DONE — it was already implemented in 2026.09.23.9; item 3 🟢 GREENLIT BUT ON HOLD — RECON questions sent to the author 2026-09-23, DO NOT START until answered.
...
- **Item 3 (author, verbatim):** "Panel Selection: This one has a lot of moving parts and a lot of verbiage from me, so definitely ask me clarifying questions." — the full spec (checkboxes on panels, Select All, visual highlight, shift-click range select, explicit deselect, selected panels' Panel-menu buttons acting on the whole selection for Duplicate / Add Panel / Move / Generate / Generate All From Here / Clear / Clear Images / Delete with an Ok / Cancel / Only-This-Panel warn dialog, optional Copy / Cut / Paste, multi-panel full-page reflow, duplicate-pairs placement vs contiguous paste, delete-with-refill Yes / No / Cancel, and a possible later phase for multi-image selection).
  - **Notes:** condensed here — the full message is in that session's conversation. Sent the author a set of RECON questions (checkbox placement + shift-click semantics, selection lifecycle, what the Panel menu should do when a selection exists vs. the clicked panel, Copy/Cut/Paste placement rules, multi-panel full-page reflow confirmations, delete-refill default, keyboard/undo expectations, and mobile/coarse-pointer behaviour). Implementation is blocked on those answers.
