...
- **Status:** ✅ **DELIVERED 2026.09.26.3** (docs + form + handoff notes pushed to the repo; no change to how the app behaves). **WAITING ON THE AUTHOR:** the eight R-tickets below are the input to any refactoring, and **no refactor starts until they are answered**.
...
- **HOW TO ANSWER:** the form is `src/refactor-form.html`; open it on the preview page with `window.__openRefactorForm()` (a ✕-closable overlay; `window.openDevForm(file, title)` opens any other `src/` html the same way). Answers draft-save in this browser as you type; **Send answers** writes one markdown file per ticket plus an updated `tickets/INDEX.md` into the repo through the same GitHub token the backup feature uses.
