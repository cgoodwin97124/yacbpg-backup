...
- Every `select` question carries a **"(recommended)"** option, so the author can legitimately answer "all defaults". A `checkboxes` question is tick-many; `text` is free-form; `notes: false` suppresses the note box.
